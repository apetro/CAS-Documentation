...
namespace :epub do
	...
	INDEX_FILEPATH = File.join(TMP_DIR, 'casmanual.html')
	...
	file INDEX_FILEPATH => HTML_FILES do
		index_file = File.open(INDEX_FILEPATH, 'w') do |file|
			...
			file << '<head>'
			file << '<title>Jasig CAS Server Manual</title>'
			file << '</head>'
			file << '<body>'
			file << "\n"
	...
	file TARGET_FILEPATH => INDEX_FILEPATH do
		opts = [
			'--language', $lang,
			'--authors', 'Jasig',
			'--comments', 'Licensed under the Apache 2.0 License',
			
			'--extra-css', 'epub/cas-manual.css',
